...
test("notas: um texto e uma lista de tarefas no espaço da pessoa, marcar item, fixar, e nada vaza entre contas", async () => {
  const owner = await register("Fefê", "fefe_notas");

  // vazio no começo
  const empty = await call("GET", "/api/notes", { session: owner });
  assert.equal(empty.response.status, 200);
  assert.deepEqual((empty.body as { notes: unknown[] }).notes, []);

  // uma nota de texto
  const text = await call("POST", "/api/notes", { session: owner, body: { kind: "text", title: "Ideia para o clube do livro", body: "Convidar a Ana.\n\n> talvez em março" } });
  assert.equal(text.response.status, 201, JSON.stringify(text.body));
  const textNote = text.body as { id: string; kind: string; title: string; body: string; items: unknown[]; colorTag: string | null; pinned: boolean };
  assert.equal(textNote.kind, "text");
  assert.equal(textNote.body, "Convidar a Ana.\n\n> talvez em março");
  assert.deepEqual(textNote.items, []);
  assert.equal(textNote.colorTag, null);
  assert.equal(textNote.pinned, false);

  // uma lista de tarefas, com uma cor
  const list = await call("POST", "/api/notes", {
    session: owner,
    body: { kind: "checklist", title: "Antes da viagem", colorTag: "amber", items: [{ text: "Passaporte" }, { text: "Carregador" }] },
  });
  assert.equal(list.response.status, 201, JSON.stringify(list.body));
  const listNote = list.body as { id: string; items: Array<{ id: string; text: string; done: boolean }>; colorTag: string };
  assert.equal(listNote.items.length, 2);
  assert.equal(listNote.items.every((item) => item.done === false), true);
  assert.equal(listNote.colorTag, "amber");

  // sem título: recusado
  const noTitle = await call("POST", "/api/notes", { session: owner, body: { kind: "text", title: "  " } });
  assert.equal(noTitle.response.status, 400);

  // lista, ordenada por atualização — a mais recente (a lista) primeiro
  const afterCreate = (await call("GET", "/api/notes", { session: owner })).body as { notes: Array<{ id: string }> };
  assert.deepEqual(afterCreate.notes.map((note) => note.id), [listNote.id, textNote.id]);

  // marcar um item da lista — sem reenviar o resto
  const toggled = await call("POST", `/api/notes/${listNote.id}/toggle`, { session: owner, body: { itemId: listNote.items[0].id, done: true } });
  assert.equal(toggled.response.status, 200, JSON.stringify(toggled.body));
  const toggledItems = (toggled.body as { items: Array<{ id: string; done: boolean }> }).items;
  assert.equal(toggledItems.find((item) => item.id === listNote.items[0].id)?.done, true);
  assert.equal(toggledItems.find((item) => item.id === listNote.items[1].id)?.done, false);

  // fixar a nota de texto — passa a vir primeiro mesmo sendo mais antiga
  const pinned = await call("PATCH", `/api/notes/${textNote.id}`, { session: owner, body: { pinned: true } });
  assert.equal(pinned.response.status, 200, JSON.stringify(pinned.body));
  const afterPin = (await call("GET", "/api/notes", { session: owner })).body as { notes: Array<{ id: string; pinned: boolean }> };
  assert.deepEqual(afterPin.notes.map((note) => note.id), [textNote.id, listNote.id]);
  assert.equal(afterPin.notes[0].pinned, true);

  // trocar só o título não mexe no corpo
  const retitled = await call("PATCH", `/api/notes/${textNote.id}`, { session: owner, body: { title: "Ideia para o clube (v2)" } });
  assert.equal((retitled.body as { title: string; body: string }).title, "Ideia para o clube (v2)");
  assert.equal((retitled.body as { title: string; body: string }).body, "Convidar a Ana.\n\n> talvez em março");

  // outra conta não vê, não edita, não marca e não apaga a nota de alguém
  const stranger = await register("Gil", "gil_notas");
  assert.deepEqual((await call("GET", "/api/notes", { session: stranger })).body, { notes: [] });
  assert.equal((await call("PATCH", `/api/notes/${textNote.id}`, { session: stranger, body: { title: "invasão" } })).response.status, 404);
  assert.equal((await call("POST", `/api/notes/${listNote.id}/toggle`, { session: stranger, body: { itemId: listNote.items[0].id, done: false } })).response.status, 404);
  assert.equal((await call("DELETE", `/api/notes/${textNote.id}`, { session: stranger })).response.status, 404);

  // apagar é reversível no banco (soft-delete) mas some da lista da pessoa
  const deleted = await call("DELETE", `/api/notes/${listNote.id}`, { session: owner });
  assert.equal(deleted.response.status, 200, JSON.stringify(deleted.body));
  const afterDelete = (await call("GET", "/api/notes", { session: owner })).body as { notes: Array<{ id: string }> };
  assert.deepEqual(afterDelete.notes.map((note) => note.id), [textNote.id]);
