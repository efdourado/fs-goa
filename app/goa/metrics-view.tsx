...
export function MetricBlock({ metric, hideThinLabel = false }: { metric: Metric; hideThinLabel?: boolean }) {
  ...
  const visibleRows = chronological ? series ?? [] : ranked;
  return (
    <article className="min-w-0 py-2">
      ...
      {series?.length ? <>
        ...
        {!chronological && pending.length ? <details className="mt-3"><summary className="cursor-pointer py-2 text-xs text-[var(--muted)]">{t("pendingRows", { count: pending.length })}</summary><ul>{pending.map(row)}</ul></details> : null}
