...
export function StatusMessage({
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      {error ? <CircleExclamationIcon className="mt-1 h-4 w-4 flex-none" /> : <CircleCheckIcon className="mt-1 h-4 w-4 flex-none" />}
      <span className="flex-1">{message}</span>
      <button
        type="button"
        className="mt-1 flex-none rounded-full opacity-60 transition hover:opacity-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--main)]/25"
    ...
    <header className="sticky top-0 z-30 border-b border-[var(--edge)] bg-[var(--canvas)]/92 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:h-[76px] sm:px-6">
        ...
        <div className="flex min-w-0 items-center gap-0.5 sm:gap-2">
          ...
          <HeaderOverflowMenu isPlatformAdmin={Boolean(user.platformAdmin)} busy={busy} onHome={onHome} onOpenPersonalSpace={onOpenPersonalSpace} onWarmPersonalSpace={onWarmPersonalSpace} onOpenTemplates={onOpenTemplates} onOpenAbout={onOpenAbout} onLogout={async () => { setBusy(true); try { await onLogout(); } finally { setBusy(false); } }} />
        </div>
      </div>
    </header>
  );
    ...
    <div className="relative sm:hidden" ref={containerRef}>
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-52 rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-1.5 shadow-[var(--elevate-2)]" role="dialog" aria-label={t("menu")}>
          ...
          <button className={cx(itemClass, "w-full text-left")} type="button" onClick={() => { onOpenPersonalSpace(); setOpen(false); }}>{t("mySpace")}</button>
