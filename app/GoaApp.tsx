...
export default function GoaApp() {
  ...
  if (user.deactivated) {
    ...
    content = <PersonalSpaceScreen challenges={bootstrap.challenges.filter((challenge) => isPersonalChallenge(challenge, bootstrap.personalWorkspaceId))} onBack={goUp} backLabel={backLabel} onOpenChallenge={(id) => openParticipant(id)} onOpenAdmin={(id) => openAdmin(id)} onCreateChallenge={() => setScreen({ kind: "create-personal-challenge" })} onOpenCatalog={() => setScreen({ kind: "personal-catalog" })} onOpenCatalogItem={(itemId) => setScreen({ kind: "personal-catalog-item", itemId })} />;
