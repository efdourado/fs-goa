...
export default function GoaApp() {
  ...
  if (screen.kind === "account") {
    ...
    content = <CreateChallengeScreen key="personal" personal onBack={() => setScreen({ kind: "dashboard" })} onCreate={(input) => createChallenge({ personal: true }, input)} />;
    ...
    content = <ParticipantChallengeScreen key={selectedChallenge.id} challenge={selectedChallenge} entries={entries} user={user} tab={screen.tab} onTab={(tab) => setScreen({ ...screen, tab })} onBack={() => selectedGroup ? setScreen({ kind: "group", groupId: selectedGroup.id }) : setScreen({ kind: "dashboard" })} onAdmin={canManage(selectedRole) ? () => openAdmin(selectedChallenge.id) : undefined} onSaveEntry={saveEntry} onDeleteEntry={(entryId) => mutateChallenge(API_PATHS.entry(entryId), undefined, "DELETE")} />;
