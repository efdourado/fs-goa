...
export default function GoaApp() {
  ...
  const personalWorkspaceId = bootstrap.personalWorkspaceId;
  ...
  // What "back" actually leads to: the group it came from, "My space" for a
  // personal challenge, or the dashboard — never a stale "Home" once the
  // button stopped meaning that.
  function backLabelFor(challenge?: { scope?: "personal" | "group"; groupId?: Id | null }): string {
    if (selectedGroup) return selectedGroup.name;
    if (challenge && isPersonalChallenge(challenge, personalWorkspaceId)) return tPersonal("title");
    return tc("home");
  }
  ...
  if (screen.kind === "account") {
    ...
    content = <CreateChallengeScreen key={selectedGroup.id} group={selectedGroup} onBack={() => setScreen({ kind: "group", groupId: selectedGroup.id })} backLabel={selectedGroup.name} onCreate={(input) => createChallenge({ groupId: selectedGroup.id }, input)} />;
  } else if (screen.kind === "create-personal-challenge") {
    content = <CreateChallengeScreen key="personal" personal onBack={() => setScreen({ kind: "personal-space" })} backLabel={tPersonal("title")} onCreate={(input) => createChallenge({ personal: true }, input)} />;
