...
export const challenges = pgTable(
  "challenges",
  {
    ...
    // The public showcase link. `result_share_token_hash` indexes the lookup at
    // /results/<token>; `result_share_token` keeps the raw token so the link can
    // be shown again (in the admin, and to participants) instead of only once at
    // publish time. Both are cleared on unpublish; a rotate replaces both.
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.recipeKey} is null or ${table.recipeKey} in ('cinema', 'library', 'bookshelf', 'habit', 'custom', 'tables', 'cine_free', 'cine_curated', 'reading_club', 'reading_daily')`,
    ),
    ...
    check(
      "challenges_raw_share_token_check",
      sql`${table.resultShareToken} is null or (${table.resultsPublishedAt} is not null and ${table.resultShareToken} ~ '^[A-Za-z0-9_-]{43}$')`,
    ),
  ],
);
